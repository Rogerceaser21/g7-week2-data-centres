...
#
# By default this script publishes g7-week2-data-centres.html exactly as it
...
DECK=g7-week2-data-centres.html
FROM_DOWNLOADS=0
FORCE=0
MSG="deck update"
...
if [ "$FROM_DOWNLOADS" -eq 1 ]; then
  NEWEST=$(ls -t "$HOME"/Downloads/g7-week2-data-centres*.html 2>/dev/null | head -1 || true)
  if [ -z "$NEWEST" ]; then
    echo "--from-downloads was asked for, but there is no g7-week2-data-centres*.html in ~/Downloads."
    exit 1
  fi
